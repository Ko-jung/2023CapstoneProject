...
void ABuilding::SetFloorAndCreate(int32 GetFloor)
{
	...
	for (int i = 0; i < Floor; ++i)
	{
		{ // 바닥 만들기
			...
			Building_Floor.AddDefaulted();
			int32 index = Building_Floor.Num() - 1;
			Building_Floor[index] = NewObject<UStaticMeshComponent>(this, UStaticMeshComponent::StaticClass(), FName(title));
			if(Building_Floor[index])
			{
				Building_Floor[index]->RegisterComponent();
				Building_Floor[index]->AttachToComponent(RootComponent.Get(), FAttachmentTransformRules::KeepRelativeTransform);
				Building_Floor[index]->SetRelativeLocation(FVector(0.0f, 0.0f, i * FloorDistance - 500.0f));

				static FSoftObjectPath MyFloorAsset(TEXT("/Script/Engine.StaticMesh'/Game/2019180031/Blueprints/Map/Building/building_floor.building_floor'"));
				UStaticMesh* FloorSM = Cast<UStaticMesh>(MyFloorAsset.ResolveObject());
				if (!FloorSM)
				{
					FloorSM = CastChecked<UStaticMesh>(MyFloorAsset.TryLoad());
				}
				if(FloorSM)
				{
					Building_Floor[index]->SetStaticMesh(FloorSM);
				}
			}
		}
		...
		{ // 벽 4개
			for(int j=0; j<4; ++j)
			{
				FString title = "Wall" + FString::FromInt(i) + "_" + FString::FromInt(j);
				TArray<UStaticMeshComponent*>* pArray{};
				UStaticMesh* StaticMeshData{};
				switch (j)
				{
				case 0:
					{
						pArray = &Building_Wall1;
						static FSoftObjectPath Wall1Asset(TEXT("/Script/Engine.StaticMesh'/Game/2019180031/Blueprints/Map/Building/building_wall001.building_wall001'"));
						StaticMeshData = Cast<UStaticMesh>(Wall1Asset.ResolveObject());
						if (!StaticMeshData)
						{
							StaticMeshData = CastChecked<UStaticMesh>(Wall1Asset.TryLoad());
						}
					}
				...
				case 1:
					{
						pArray = &Building_Wall2;
						static FSoftObjectPath Wall2Asset(TEXT("/Script/Engine.StaticMesh'/Game/2019180031/Blueprints/Map/Building/building_wall002.building_wall002'"));
						StaticMeshData = Cast<UStaticMesh>(Wall2Asset.ResolveObject());
						if (!StaticMeshData)
						{
							StaticMeshData = CastChecked<UStaticMesh>(Wall2Asset.TryLoad());
						}
					}
				...
				case 2:
					{
						pArray = &Building_Wall3;
						static FSoftObjectPath Wall3Asset(TEXT("/Script/Engine.StaticMesh'/Game/2019180031/Blueprints/Map/Building/building_wall003.building_wall003'"));
						StaticMeshData = Cast<UStaticMesh>(Wall3Asset.ResolveObject());
						if (!StaticMeshData)
						{
							StaticMeshData = CastChecked<UStaticMesh>(Wall3Asset.TryLoad());
						}
					}
					break;
				case 3:
					{
						pArray = &Building_Wall4;
						static FSoftObjectPath Wall4Asset(TEXT("/Script/Engine.StaticMesh'/Game/2019180031/Blueprints/Map/Building/building_wall004.building_wall004'"));
						StaticMeshData = Cast<UStaticMesh>(Wall4Asset.ResolveObject());
						if (!StaticMeshData)
						{
							StaticMeshData = CastChecked<UStaticMesh>(Wall4Asset.TryLoad());
						}
					}
					break;
				default:
				...
				pArray->AddDefaulted();
				int32 index = pArray->Num() - 1;
				(*pArray)[index] = NewObject<UStaticMeshComponent>(this, UStaticMeshComponent::StaticClass(), FName(title));
				if ((*pArray)[index])
				{
					(*pArray)[index]->RegisterComponent();
					(*pArray)[index]->AttachToComponent(RootComponent.Get(), FAttachmentTransformRules::KeepRelativeTransform);
					(*pArray)[index]->SetRelativeLocation(FVector(0.0f, 0.0f, i * FloorDistance));
					
					if (StaticMeshData)
					{
						(*pArray)[index]->SetStaticMesh(StaticMeshData);
					}
				}
